...
* 进程信息：查看当前进程列表，签名状态等，可以结束进程及进程树。
* 网络信息：查看当前主机的网络请求及发起的进程，增加可疑网络行为的排查。
...
(2).未显示微步数据说明API没有余额了
(3).这里调用的微步API只支持查询IP，不支持查询域名（普通用户API，每月仅可查询20个IP，可尝试注册几个账号，（这里lm注册了6个账号，每月查询120个IP）
...
可点击列名，内容将会排序，右键有专门的菜单栏

![](_v_images/1745855761_24573.png)

![](_v_images/1745856082_10591.png)

![](_v_images/1745889078_1880.png)
...
* **进程信息查看:**
以挖矿和远控为例，可以查看进程注入类的攻击手法，结束掉相关进程和进程输入，以及父进程

![](_v_images/1745856266_13010.png)
...
* **网络信息查看:**
以外连或横向扫描为例，可以查看可疑的网络请求，定位该进程和对应程序。结合进程信息可分析是否存在父进程

![](_v_images/1745855486_21416.png)
...
![](_v_images/1745856054_31246.png)
